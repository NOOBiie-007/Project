...
class _AprilState extends State<April> {
  ...
  totalmoney() async {
    int total = 0;
    var snapshot = await Firestore.instance.collection("Aprili").getDocuments();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Income in April"),
        backgroundColor: Colors.purple,
      ),
      body: StreamBuilder(
        stream: Firestore.instance.collection("Aprili").snapshots(),
        builder: (_, snapshot) {
          if (!snapshot.hasData) {
            return Center(
              child: Text("Loading"),
      ...
      bottomNavigationBar: Container(
          ...
          child: ListTile(
            title: Text('        Total Income In April',
