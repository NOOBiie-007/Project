...
class _SeptemberState extends State<September> {
  ...
  totalmoney() async {
    int total = 0;
    var snapshot =
        await Firestore.instance.collection("Septemberi").getDocuments();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Income in September"),
        backgroundColor: Colors.purple,
      ),
      body: StreamBuilder(
        stream: Firestore.instance.collection("Septemberi").snapshots(),
        builder: (_, snapshot) {
          if (!snapshot.hasData) {
            return Center(
              child: Text("Loading"),
      ...
      bottomNavigationBar: Container(
          ...
          child: ListTile(
            title: Text('        Total Income In September',
