...
class _AugustState extends State<August> {
  ...
  totalmoney() async {
    int total = 0;
    var snapshot =
        await Firestore.instance.collection("Augusti").getDocuments();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Income in August"),
        backgroundColor: Colors.purple,
      ),
      body: StreamBuilder(
        stream: Firestore.instance.collection("Augusti").snapshots(),
        builder: (_, snapshot) {
          if (!snapshot.hasData) {
            return Center(
              child: Text("Loading"),
      ...
      bottomNavigationBar: Container(
          ...
          child: ListTile(
            title: Text('        Total Income In August',
