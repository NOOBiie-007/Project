...
class October extends StatefulWidget {
  @override
  _OctoberState createState() => _OctoberState();
}
...
class _OctoberState extends State<October> {
  ...
  totalmoney() async {
    int total = 0;
    var snapshot =
        await Firestore.instance.collection("Octoberi").getDocuments();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Income in October"),
        backgroundColor: Colors.purple,
      ),
      body: StreamBuilder(
        stream: Firestore.instance.collection("Octoberi").snapshots(),
        builder: (_, snapshot) {
          if (!snapshot.hasData) {
            return Center(
              child: Text("Loading"),
      ...
      bottomNavigationBar: Container(
          ...
          child: ListTile(
            title: Text('        Total Income In October',
